...
public class StaticWeatherAnalyze {
    public static String getCityField(JSONObject json) {
        try { return json.getString("name").toUpperCase() + ", " + json.getJSONObject("sys").getString("country");
        } catch (JSONException e) { e.printStackTrace();
        }
        return "NaN";
    }
    public static String getLastUpdateTime(JSONObject json) {
        try { DateFormat df = DateFormat.getDateTimeInstance();
    ...
    public static String getDetailsField(JSONObject json) {
        try { JSONObject details = json.getJSONArray("weather").getJSONObject(0);
        ...
        try { JSONObject main = json.getJSONObject("main");
